...
namespace SourceGit.Utilities
{
    ...
    public static class OFPAParser
    {
        ...
        internal const int MaxSampleSize = 256 * 1024;
        private const int MaxNameMapEntries = 100000;
        ...
        public static DecodeResult? Decode(string filePath)
        {
            try
            {
                var data = OFPAFilePrefixReader.Read(filePath, MaxSampleSize);
                if (data == null)
                    return null;
        ...
        private static DecodeResult? ParseUAsset(byte[] buffer)
        {
            ...
            int slashOffset = FindByte(buffer, (byte)'/', MinimumHeaderSize, Math.Min(fileSize, MaxSampleSize));
            if (slashOffset >= 24)
            {
            ...
            int headerLength = Math.Min(fileSize, MaxSampleSize);
            ...
            for (int currentPos = searchStart; currentPos < scanLimit; currentPos++)
            {
                ...
                if (stringLength > 0 && stringLength < MaxStringLength)
                {
                    ...
                    // Check for '/' or "None"
                    if (firstChar != ForwardSlashByte && !(firstChar == UpperCaseN && MatchBytes(buffer, currentPos + 4, "None")))
                        continue;
                    ...
                    if (count > 0 && count < MaxNameMapEntries && offset > 0 && offset < fileSize)
                    {
                        nameCount = count;
                        nameOffset = offset;
                        break;
            ...
            for (int i = valueSearchStart; i < valueSearchEnd - 4; i++)
            {
                ...
                if (stringLength > 0 && stringLength < 128)
                {
                    ...
                    {
                        if (stringEnd > i + 4 && IsAsciiPrintable(buffer, i + 4, stringEnd))
                        {
        ...
        private static bool IsAsciiPrintable(byte[] buffer, int start, int end)
        {
            for (int i = start; i < end; i++)
            {
                byte b = buffer[i];
